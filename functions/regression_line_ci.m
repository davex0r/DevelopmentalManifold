function [top_int, bot_int, X, Y] = regression_line_ci(alpha,beta,x,y,varargin)
...
SE_y_cond_x = nansum((y - beta(1)*ones(size(y))-beta(2)*x).^2)/(N-2);
SSX = (N-1)*nanvar(x);
SE_Y = SE_y_cond_x*(ones(size(X))*(1/N + (nanmean(x)^2)/SSX) + (X.^2 - 2*nanmean(x)*X)/SSX);
